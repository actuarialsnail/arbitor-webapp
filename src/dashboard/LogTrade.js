...
export default function Opports() {
  const [tradeLogs, setTradeLogs] = React.useState([]);
  ...
  const processTradeLogs = date => {
    ...
    cancelLogsListener();
    requestLogs(date, 'execution', (err, logs) => {
      ...
      for (const log of logs) {
        if (log.type === 'execution') { tradeLogs.push(log) };
      };
          ...
          columns={[
            ...
            },
            {
              title: 'Details', field: 'route', render: rowData => {
                return (
                  <DialogTrade tradeLog={rowData}></DialogTrade>
                )
              }, sorting: false
            },
          ]}
