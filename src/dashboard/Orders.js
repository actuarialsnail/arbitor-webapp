        ...
        <TableBody>
          ...
          .map(row => (
              <TableRow key={row.tradeObj.route.join(':')+row.timestamp}>
                <TableCell>{row.tradeObj.route.map((v,i) => {return(<div key={i}>{v}</div>)})}</TableCell>
                <TableCell align="right">{formatCell(row.tradeObj.price).map((v,i) => {return(<div key={i}>{v}</div>)})}</TableCell>
                <TableCell align="right">{formatCell(row.tradeObj.mktSize).map((v,i) => {return(<div key={i}>{v}</div>)})}</TableCell>
                <TableCell align="right">{formatCell(row.tradeObj.accSize).map((v,i) => {return(<div key={i}>{v}</div>)})}</TableCell>
                <TableCell align="right">{Number.parseFloat(row.tradeObj.balanceFilteredValue).toFixed(8)}</TableCell>
                <TableCell>
                  <Log verificationLog={row} executionLog={executionLogs[row.tradeObj.route.join(':')+row.timestamp]}></Log>
                </TableCell>
              </TableRow>
          ))}
        </TableBody>
